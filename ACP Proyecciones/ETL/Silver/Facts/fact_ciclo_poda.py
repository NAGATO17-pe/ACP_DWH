...
class ProcesadorCicloPoda(BaseFactProcessor):
    ...
    def _construir_payload(self, df: pd.DataFrame) -> list[dict]:
        ...
        for _, fila in df.iterrows():
            ...
            # Punto de muestreo: clave del grain, obligatorio.
            punto = _a_entero(fila.get('Punto_Raw'))
            if punto is None:
                # Sin Punto valido no podemos respetar el grain; saltar fila.
                # (el helper de duplicados ya consolida; aqui evitamos colisiones
                #  en el UNIQUE INDEX filtrado WHERE Punto IS NOT NULL)
                continue
...
def cargar_fact_ciclo_poda(engine: Engine) -> dict:
    ...
    with ContextoTransaccionalETL(engine) as contexto:
        ...
        df = proc.pre_limpiar_duplicados_batch(df, ['Modulo_Raw', 'Fecha_Raw', 'Variedad_Raw', 'Tipo_Evaluacion_Raw', 'Punto_Raw'])
